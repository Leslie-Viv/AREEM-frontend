...
export class EditarEmpresaComponent {
  ...
  regresar(): void {
    this.dialogRef.close(); // Cerrar el diálogo sin realizar cambios
  }

  actualizarEmpresa() {
    const datosActualizados = this.empresaForm.value;
    this.empresaS.actualizarempresa(this.empresa.id, datosActualizados).subscribe(
      response => {
        console.log('Empresa actualizada correctamente', response);
        Swal.fire({
          icon: 'success',
        ...
        this.dialogRef.close(); // Cerrar el diálogo después de la actualización
      },
      error => {
        console.error('Error al actualizar empresa', error);
        Swal.fire({
