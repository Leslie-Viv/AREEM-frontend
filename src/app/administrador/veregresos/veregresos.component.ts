...
export class VeregresosComponent {
  ...
  generatePDF(egresos: any[]): void {
    try {
      const documentDefinition = {
        ...
        content: [
          { text: 'Lista de Egresos', style: 'header' },
          '\n\n',
          {
            table: {
              headerRows: 1,
      ...
      const fileName = `Lista_de_Egresos_${formattedDate}.pdf`;
  ...
  editaregreso(egreso: Egresos): void {
    this.router.navigate(['/editar-egreso', egreso.id]);
  }
